...
/**
 * Provides a Node Import-Export function.
 */
class NodeExport {
  ...
  protected static function getFileUri($format) {
    return uniqid(file_default_scheme() . '://node_import_export_') . '.' . strtolower($format);
  }
        ...
        if(stripos($key, 'field_') === 0) {
          foreach ($nodesArray[$index][$key] as $idx => $v) {
            if (isset($v['target_id'])) {
              ...
              $nodesArray[$idx]['_metadata']['node'][$v['target_id']] = $node_reference->get('uuid')->value;
            }
          }
        }
        // @todo Expand to support User Ids (uid) for uid & revision_uid
      }
      $index++;
    }
    return $nodesArray;
  ...
  public static function nodeExportFinishedCallback($success, array $results, array $operations) {
    ...
    if ($success) {
      $message = \Drupal::translation()->formatPlural(
        count($results),
        'One Node Import-Exported ', '@count nodes exported '
      );
    }
    else {
