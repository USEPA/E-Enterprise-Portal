(function ($) {
    ...
    function processPageAnchors() {
        ...
        // process anchor tags
        $('.panel-pane:not(' + ignore_panels.join(',') + ') a:not(.favorites-ignore, .menu-link, .skip-link, .paginate_button, [href^=mailto], [href^=javascript], .remove-link)').each(function () {
...
// Ctrl + D (68) toggles the favorite and shows the qTip
        $("a").not(".menu-link", ".ctools-use-modal", ".skip-link", ".favorites-ignore", ".paginate_button", "[href^=mailto]", "[href^=javascript]", ".remove-link").focus(function () {
            $("a").not(".menu-link", ".ctools-use-modal", ".skip-link", ".favorites-ignore", ".paginate_button", "[href^=mailto]", "[href^=javascript]", ".remove-link").keydown(function (event) {
                try {
                    if (event.which === 16) {
                        if (event.which === 9) {
