...
var yadcf = (function($) {
  ...
  var parent_column_number = -1;

  function doFilter(arg, table_selector_jq_friendly, column_number, filter_match_mode, event) {
    if (event.currentTarget) {
      parent_column_number = column_number;
    }
    ...
    var table_options = getOptions(oTable.selector);
    // Return highest key value for column, or the last column to be filtered
    var last_column = Object.keys(table_options).reduce(function(a, b) {
      return table_options[a] > table_options[b] ? a : b
    });
      ...
      if (args.hasOwnProperty(columnObjKey)) {
        ...
        if ($filter_selector.length === 1) {
          ...
          if (columnObj.filter_type === "select" || columnObj.filter_type === 'custom_func') {
            ...
            if (columnObj.filter_type === "select") {
              filterActionStr = 'onchange="yadcf.doFilter(this, \'' + table_selector_jq_friendly + '\', ' + column_number + ', \'' + filter_match_mode + '\', event);"';
              if (columnObj.externally_triggered === true) {
                filterActionStr = '';
              }
