...
( /** @lends <global> */ function($, window, document, undefined) {
    ...
    var DataTable = function(oInit) {
        ...
        function _fnAddColumn(oSettings, nTh) {
            ...
            var oCol = $.extend({}, DataTable.models.oColumn, oDefaults, {
                ...
                "aDataSort": oDefaults.aDataSortno ? oDefaults.aDataSort : [iCol],
        ...
        function _fnBuildHead(oSettings) {
            var i, nTh, iLen, j, jLen, sortingLink;
            ...
            if (iThs !== 0) {
                ...
                for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
                    ...
                    if (oSettings.aoColumns[i].bSortable) {
                        /* There is no advantage to having both the column header and the link inside of it
                        part of the tab order, it should be one or ther other. If the grid itself is navigable
                        (e.g. like a spreadsheet style grid), the header should be focusable and focusable through 
                        arrow key nav. For regular data tables it should be the link inside of it. The aria-controls
                        attrib is semantically correct, but does not really achieve anything besides overly verbose 
                        screen reader announcement. It's clear enough without aria-controls that a sortable column 
                        heard controls the grid it is placed in. Removing aria-controls unclutters the screen reader
                        output*/
                    ...
                    $(nTh).wrapInner("<a class='sortingLink' href='#'></a>");
                    sortingLink = $(nTh).children(".sortingLink");

                    if (oSettings.aoColumns[i].bSortable) {
                        $(sortingLink).append(" <img class='headerSortIcon' src='" + oSettings.sImgDirPath + "sort_both.png' alt=''/>");
                    }
                }
            } else {
                ...
                for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
                    ...
                    sortingLink = $("<a href='#'></a>");
                    $(nTh).wrapInner(sortingLink);
            ...
            if (oSettings.bJUI) {
                for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
                    ...
                    nLink.className = oSettings.oClasses.sSortJUIWrapper;
                    $(nTh).contents().appendTo(nLink);

                    var nSpan = document.createElement('span');
                    nSpan.className = oSettings.oClasses.sSortIcon;
                    nLink.appendChild(nSpan);
                    nTh.appendChild(nLink);
                }
            }
        ...
        function _fnAddOptionsHtml(oSettings) {
             ...
             */
            oSettings.nTableWrapper = $('<div id="' + oSettings.sTableId + '_wrapper" class="' + oSettings.oClasses.sWrapper + '"></div>')[0];
            ...
            for (var i = 0; i < aDom.length; i++) {
                ...
                if (cOption == '<') {
                    ...
                    if (cNext == "'" || cNext == '"') {
                        ...
                        if (sAttr == "H") {
                            ...
                            sAttr = "fg-toolbar ui-toolbar ui-corner-bl ui-corner-br ui-helper-clearfix";
                        }
                ...
                if (iPushFeature == 1 && nTmp !== null) {
                    ...
                    // Only add the live region once --- removed for region already in the page
                    /*  if (i === 0) {
                        $(nInsertNode).append("<div class='sr-only' role='status' aria-live='polite' id='" + oSettings.sTableId + '_liveRegion' + "'></div>");
                    }*/
                }
            }
        ...
        function _fnFeatureHtmlFilter(oSettings) {
            ...
            sSearchStr = (sSearchStr.indexOf('_INPUT_') !== -1) ?
                sSearchStr.replace('_INPUT_', '<input class="fSearch" type="text" id="dataTableSearch" />') :
                sSearchStr === "" ? '<input class="fSearch" type="text" id="dataTableSearch" />' : sSearchStr + ' <input class="fSearch" type="text" id="dataTableSearch" />';


            var nFilter = document.createElement('div');
            nFilter.setAttribute('role', 'search');
            nFilter.className = oSettings.oClasses.sFilter;
            nFilter.innerHTML = '<label for="dataTableSearch" class="table_Label"> Search </label>' + sSearchStr;
        ...
        function _fnFeatureHtmlInfo(oSettings) {
            ...
            if (!oSettings.aanFeatures.i) {
                ...
                nInfo.setAttribute("aria-live", "polite");
            }
            //oSettings.nTable.setAttribute('role', 'grid');
        ...
        function _fnFeatureHtmlLength(oSettings) {
            ...
            var sStdMenu = '<select size="1" id="showEntries" ' + sName + '>';
            var i, iLen;
            ...
            nLength.innerHTML = '<label for="showEntries" class="table_Label"> Show Entries </label>' + oSettings.oLanguage.sLengthMenu.replace('_MENU_', sStdMenu);
        ...
        function _fnSort(oSettings, bApplyClasses) {
            ...
            for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
                ...
                if (aoColumns[i].bSortable) {
                    if (aaSort.length > 0 && aaSort[0][0] == i) {
                        ...
                        $(nTh).find("img.headerSortIcon")
                            .attr("alt", oAria.sSortableColumn + ", " + (nextSort == "asc" ? oAria.sSortedDescending + ", " + oAria.sSortAscending : oAria.sSortedAscending + ", " + oAria.sSortDescending));
                        //$(nTh).attr('title', (nextSort == "asc" ? oAria.sSortedDescending + ", " + oAria.sSortAscending : oAria.sSortedAscending + ", " + oAria.sSortDescending));
                        /* These lines have been commented out, as it was causing navigation to start on the actively 
                        sorted column, rather than the start of the page in Chrome and Safari */
                        //$(nTh).blur();
                        //$(nTh).focus();
                    } else {
                        nTh.setAttribute('aria-label', sTitle);
                        ...
                        //$(nTh).find('a').attr('title', oAria.sSortableColumn + ", " + oAria.sSortAscending);
                        $(nTh).find("img.headerSortIcon")
                            .attr("alt", oAria.sSortableColumn + ", " + oAria.sSortAscending);
                    }
                } else {
                    nTh.setAttribute('aria-label', sTitle);
                    $(nTh).find('a').removeAttr('title');
                }

                //DYNAMICALLY ADD ABBREVIATION IF DOESN'T EXIST
                if (typeof $(nTh).attr('abbr') !== "string")
                    $(nTh).attr('abbr', $(nTh).text());
            }
            ...
            var lRUpdate = oAria.sSortingLiveRegionUpdate;
            lRUpdate = lRUpdate.replace("_COLNAME_", $(aoColumns[aaSort[0][0]].nTh).attr("abbr"));
            lRUpdate = lRUpdate.replace("_DIRECTION_", aaSort[0][1] == "asc" ? oAria.sAscending : oAria.sDescending)
            $("#liveText").text(lRUpdate);
        }
        ...
        function _fnSortingClasses(oSettings) {
            ...
            for (i = 0; i < oSettings.aoColumns.length; i++) {
                if (oSettings.aoColumns[i].bSortable) {
                    ...
                    var imgSrc = "sort_both.png";
                    if (sClass != "sorting") {
                        imgSrc = sClass == oClasses.sSortAsc ? "sort_asc.png" : "sort_desc.png";
                    }
                    $(oSettings.aoColumns[i].nTh).find("img.headerSortIcon")
                        .attr("src", oSettings.sImgDirPath + imgSrc);
         ...
         *  @memberof DataTable#oApi
         */
        function _fnBindAction(n, oData, fn, reFocus) {
            $(n)
                .bind('click.DT', oData, function(e) {
                    // commenting this out because it's causing issues with loss of keyboard focus 
                    //n.blur(); // Remove focus outline for mouse users

                    // When JAWS is running, this event is reached when a key is pressed instead of 
                    // the keypress event, so refocus logic here for JAWS.
                    if (reFocus) {
                        DataTable.ext.bReFocus = true;
                    }
                    fn(e);
                })
        ...
        this.fnDestroy = function(bRemove) {
            ...
            if (oSettings.bJUI) {
                ...
                $('th, td', oSettings.nTHead).each(function() {
                    var jqWrapper = $('a.' + oSettings.oClasses.sSortJUIWrapper, this);
        ...
        return this.each(function() {
            ...
            _fnMap(oSettings, oInit, "sImgDirPath");
            ...
            //HH: It's not helpful to have a screen reader reannounce the entire table as it's sorted
    ...
    DataTable.defaults = {
        ...
        "oLanguage": {
             ...
             */
            "oAria": {
                "sSortableColumn": "sortable column",
                /**
                ...
                "sSortedAscending": "sorted ascending",
                "sSortedDescending": "sorted descending",
                "sAscending": "ascending",
                "sDescending": "descending",
                "sSortingLiveRegionUpdate": "Sorted column _COLNAME_ in _DIRECTION_ order",
                "sFirst": "Show first table page",
                "sLast": "Show last table page",
                "sNext": "Show next table page",
                "sPrevious": "Show previous table page",
                "sShowTablePage": "Show Table Page ",
                "sCurrentlySelected": "currently selected"
            },
             ...
             *    } );
             */
            "sInfo": "<span>Showing _START_ to _END_ of _TOTAL_ entries</span>",
             ...
             *    } );
             */
            "sInfoEmpty": "Displaying 0 to 0 of 0",
    ...
    $.extend(DataTable.ext.oStdClasses, {
        ...
        "sPageFirst": "first paginate_button_text",
        "sPagePrevious": "previous paginate_button_text",
        "sPageNext": "next paginate_button_text",
        "sPageLast": "last paginate_button_text",
    ...
    $.extend(DataTable.ext.oJUIClasses, DataTable.ext.oStdClasses, {
        ...
        /* Original style changed by MK on May 16th
        ...
        "sSortJUIAsc": "float_right ui-icon ui-icon-triangle-1-n",
        "sSortJUIDesc": "float_right ui-icon ui-icon-triangle-1-s",
        "sSortJUI": "float_right ui-icon ui-icon-carat-2-n-s",
        "sSortJUIAscAllowed": "float_right ui-icon ui-icon-carat-1-n",
        "sSortJUIDescAllowed": "float_right ui-icon ui-icon-carat-1-s",
    ...
    $.extend(DataTable.ext.oPagination, {
        ...
        "two_button": {
            ...
            "fnInit": function(oSettings, nPaging, fnCallbackDraw) {
                ...
                var sAppend = (!oSettings.bJUI) ?
                    '<a class="' + oSettings.oClasses.sPagePrevDisabled + '" tabindex="' + oSettings.iTabIndex + '" role="button" aria-label="' + oAria.sPrevious + '">' + oLang.sPrevious + '</a>' +
                    '<a class="' + oSettings.oClasses.sPageNextDisabled + '" tabindex="' + oSettings.iTabIndex + '" role="button" aria-label="' + oAria.sNext + '">' + oLang.sNext + '</a>' :
            ...
            "fnUpdate": function(oSettings, fnCallbackDraw) {
                ...
                for (var i = 0, iLen = an.length; i < iLen; i++) {
                    if (an[i].childNodes.length !== 0) {
                        ...
                        an[i].childNodes[0].setAttribute("aria-disabled", (oSettings._iDisplayStart === 0) ? "true" : "false");

                        an[i].childNodes[1].className = (oSettings.fnDisplayEnd() == oSettings.fnRecordsDisplay()) ?
                            oClasses.sPageNextDisabled : oClasses.sPageNextEnabled;
                        an[i].childNodes[1].setAttribute("aria-disabled", (oSettings.fnDisplayEnd() == oSettings.fnRecordsDisplay()) ? "true" : "false");
                    }
                }
            }
        },
        ...
        "full_numbers": {
            ...
            "fnInit": function(oSettings, nPaging, fnCallbackDraw) {
                ...
                $(nPaging).append(
                    '<a  role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButton + " " + oClasses.sPageFirst + '" aria-label="' + oAria.sFirst + '">' + oLang.sFirst + '</a>' +
                    '<a  role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButton + " " + oClasses.sPagePrevious + '" aria-label="' + oAria.sPrevious + '">' + oLang.sPrevious + '</a>' +
                    '<span></span>' +
                    '<a role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButton + " " + oClasses.sPageNext + '" aria-label="' + oAria.sNext + '">' + oLang.sNext + '</a>' +
                    '<a role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButton + " " + oClasses.sPageLast + '" aria-label="' + oAria.sLast + '">' + oLang.sLast + '</a>'
                );
                var els = $('a', nPaging);
            ...
            "fnUpdate": function(oSettings, fnCallbackDraw) {
                ...
                for (i = iStartButton; i <= iEndButton; i++) {
                    sList += (iCurrentPage !== i) ?
                        '<a role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButton + '"><span class="sr-only">' + oAria.sShowTablePage + '</span>' + oSettings.fnFormatNumber(i) + '</a>' :
                        '<a role="button" tabindex="' + oSettings.iTabIndex + '" class="' + oClasses.sPageButtonActive + '"><span class="sr-only">' + oAria.sShowTablePage + '</span>' + oSettings.fnFormatNumber(i) + '<span class="sr-only"> ,' + oAria.sCurrentlySelected + '</span></a>';
                }
                ...
                for (i = 0, iLen = an.length; i < iLen; i++) {
                    ...
                    if (DataTable.ext.bReFocus) {
                        $(anButtons).eq(iCurrentPage + 1 - (iStartButton - 1)).focus();
                        DataTable.ext.bReFocus = false;
                    }
                    anStatic = [
                    ...
                    ).attr("aria-disabled", iCurrentPage == 1 ? "true" : "false");
                    $([anStatic[2], anStatic[3]]).addClass(
                        (iPages === 0 || iCurrentPage === iPages || oSettings._iDisplayLength === -1) ?
                        oClasses.sPageButtonStaticDisabled :
                        oClasses.sPageButton
                    ).attr("aria-disabled", iPages === 0 || iCurrentPage === iPages || oSettings._iDisplayLength === -1 ? "true" : "false");
                }
            }
        }
    });
