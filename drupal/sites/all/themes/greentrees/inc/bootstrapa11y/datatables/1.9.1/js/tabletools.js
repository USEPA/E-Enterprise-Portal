...
a + "\\s*"))
    ...
    Client: function (a) { this.handlers = {}; this.id = ZeroClipboard_TableTools.nextId++; this.tabindex = "-1"; this.movieId = "ZeroClipboard_TableToolsMovie_" + this.id; ZeroClipboard_TableTools.register(this.id, this); a && this.glue(a) } 
};
...
"px"; e.height = d.height + "px"; e.zIndex = c; "undefined" != typeof b && "" != b && (this.div.title = b); 0 != d.width && 0 != d.height && (this.sized = !0); this.domElement && (this.domElement.appendChild(this.div), this.div.innerHTML = this.getHTML(d.width, d.height))
    ...
    var a = ZeroClipboard_TableTools.getDOMObjectPosition(this.domElement), b = this.div.style; b.position = "absolute"; b.left = this.domElement.offsetLeft + "px"; b.top = this.domElement.offsetTop + "px"; b.width = a.width + "px"; b.height = a.height + "px"; 0 != a.width &&
0 != a.height && (this.sized = !0, b = this.div.childNodes[0], b.width = a.width, b.height = a.height)
}, getHTML: function (a, b) {
    var c = "", d = "id=" + this.id + "&width=" + a + "&height=" + b; if (navigator.userAgent.match(/MSIE/)) var e = location.href.match(/^https/i) ? "https://" : "http://", c = c + ('<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="../../../../2.1/global/js/' + e + 'download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0" width="' + a + '" height="' + b + '" id="' + this.movieId + '" align="middle" tabindex="-1"><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="false" /><param name="movie" value="../../../../2.1/global/js/' +
ZeroClipboard_TableTools.moviePath + '" /><param name="loop" value="false" /><param name="menu" value="false" /><param name="quality" value="best" /><param name="bgcolor" value="#ffffff" /><param name="flashvars" value="' + d + '"/><param name="wmode" value="transparent"/></object>'); else c += '<embed id="' + this.movieId + '" src="../../../../2.1/global/js/' + ZeroClipboard_TableTools.moviePath + '" loop="false" menu="false" quality="best" bgcolor="#ffffff" width="' + a + '" height="' + b + '" name="' + this.movieId + '" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="' +
    ...
    TableTools.prototype = {
        ...
        */
        "_fnConstruct": function (oOpts) {
            ...
            this.dom.container.className = this.classes.container + ' DTTT_order';
            ...
            this.dom.container.setAttribute('role', 'toolbar');
            this.dom.container.setAttribute('aria-label', 'Table functions');
            this.dom.container.setAttribute('aria-activedescendant', 'ToolTables_' + this.s.dt.sInstance + '_0');
        ...
        */
        "_fnCreateButton": function (oConfig, bCollectionButton) {
            ...
            this._fnToolbar(nButton, oConfig)

            return nButton;
        },

        /**
        * Handle keystrokes for the toolbar
        *  @method  _fnToolbar
        *  @param   {Node} nButton Button element
        *  @returns void
        *  @private 
        */
        "_fnToolbar": function (nButton) {
            $(nButton).keydown(function (e) {
                var iKey = e.keyCode;
                var objToolbar = nButton.parentNode;
                var strID, iCurrent, objButton, iNew;

                if (iKey === 37 || iKey === 39) {
                    strID = objToolbar.getAttribute('aria-activedescendant');
                    iCurrent = strID.substr(strID.lastIndexOf('_') + 1);

                    if (iKey === 37) { // Left cursor key
                        iNew = parseInt(iCurrent, 10) - 1;
                    }
                    if (iKey === 39) { // Right cursor key
                        iNew = parseInt(iCurrent, 10) + 1;
                    }

                    strID = strID.replace('_' + iCurrent, '_' + iNew);
                    objButton = document.getElementById(strID);
                    if (objButton) {
                        objToolbar.setAttribute('aria-activedescendant', strID);
                        nButton.setAttribute('tabindex', '-1');
                        objButton.setAttribute('tabindex', '0');
                        objButton.focus();
                    }
                    e.preventDefault();
                }
        ...
        */
        "_fnButtonBase": function (o, bCollectionButton) {
		  ...
		  masterS = this._fnGetMasterSettings();
            ...
            // Only the active button should be in keyboard tab order

            if (masterS.buttonCounter === 0) {
                nButton.setAttribute("tabindex", "0");
            }
            else {
                nButton.setAttribute("tabindex", "-1");
            }

            nButton.setAttribute("role", "button");
